...
extension Analytics {
    ...
    func log(message: String, kind: LogFilterKind? = nil, function: String = #function, line: Int = #line) {
        apply { plugin in
    ...
    func metric(_ type: MetricType, name: String, value: Double, tags: [String]? = nil) {
        apply { plugin in
            // Check if we should send off the event
            if SegmentLog.loggingEnabled == false {
                return
    ...
    ///   - function: The name of the function the log came from. This will be captured automatically.
    ///   - line: The line number in the function the log came from. This will be captured automatically.
    func history(event: Message, sender: AnyObject, function: String = #function, line: Int = #line) {
        apply { plugin in
            // Check if we should send off the event
            if SegmentLog.loggingEnabled == false {
                return
...
extension Analytics {
    ...
    ///   API on Analytics.
    func add(target: LogTarget, type: LoggingType) {
