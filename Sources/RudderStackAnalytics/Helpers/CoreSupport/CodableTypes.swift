...
public struct AnyCodable: Codable {
    ...
    /**
     Decoding initializer required by the Codable protocol.

     AnyCodable is designed for encoding only. Use `init(_ value: Any)` to create instances.
     This method will always throw a `DecodingError` if invoked.
     */
    public init(from decoder: Decoder) throws {
        LoggerAnalytics.error("AnyCodable decoding is not supported")
        let container = try decoder.singleValueContainer()
        throw DecodingError.dataCorruptedError(in: container, debugDescription: "AnyCodable decoding is not supported")
    }
