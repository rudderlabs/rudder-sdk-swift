...
struct PrimaryRetryHeadersProviderTests {
    ...
    func testPrepareHeadersReturnsEmptyWhenNoMetadata() {
        let headers = provider.prepareHeaders(batchId: "batch-1", currentTimestampInMillis: 1000)
    ...
    func testRecordFailureThenPrepareHeadersReturnsHeaders() {
        provider.recordFailure(batchId: "batch-1", timestampInMillis: 1000, error: .retryable(statusCode: 502))

        let headers = provider.prepareHeaders(batchId: "batch-1", currentTimestampInMillis: 1500)
    ...
    func testSecondFailureIncrementsAttemptTo2() {
        provider.recordFailure(batchId: "batch-1", timestampInMillis: 1000, error: .retryable(statusCode: 502))
        provider.recordFailure(batchId: "batch-1", timestampInMillis: 2000, error: .retryable(statusCode: 502))

        let headers = provider.prepareHeaders(batchId: "batch-1", currentTimestampInMillis: 2500)
    ...
    func testSinceLastAttemptCalculation() {
        provider.recordFailure(batchId: "batch-1", timestampInMillis: 1000, error: .timeout)
    ...
    func testSinceLastAttemptClampsToZeroOnClockSkew() {
        provider.recordFailure(batchId: "batch-1", timestampInMillis: 2000, error: .timeout)
    ...
    func testRetryReasonUpdatesOnSubsequentFailure() {
        provider.recordFailure(batchId: "batch-1", timestampInMillis: 1000, error: .timeout)
        provider.recordFailure(batchId: "batch-1", timestampInMillis: 2000, error: .networkUnavailable)
    ...
    func testBatchIdMismatchReturnsEmptyHeaders() {
        ...
        let headers = provider.prepareHeaders(batchId: "batch-2", currentTimestampInMillis: 1500)
    ...
    func testBatchIdMismatchDoesNotClearMetadata() {
        ...
        let mismatchHeaders = provider.prepareHeaders(batchId: "batch-2", currentTimestampInMillis: 1500)
        #expect(mismatchHeaders.isEmpty)

        // Original batch still returns headers
        let originalHeaders = provider.prepareHeaders(batchId: "batch-1", currentTimestampInMillis: 2000)
