...
class BackoffPolicyHandlerTests {
    ...
    func testReset() async {
        let handler = BackoffPolicyHandler(policy: mockPolicy, logger: MockConstant.noOpLogger)
    ...
    func testWaitWithBackoffExceedsMaxAttempts() async {
        ...
        let handler = BackoffPolicyHandler(policy: mockPolicy, coolOffPeriodMillis: 10, logger: MockConstant.noOpLogger)
    ...
    func testIntegrationWithRealPolicy() async {
        ...
        let handler = BackoffPolicyHandler(policy: policy, logger: MockConstant.noOpLogger)
