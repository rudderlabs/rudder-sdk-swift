...
struct DeviceModeConsentRestampTests {
    ...
    func testDroppedEventBypassesRestamp() {
        ...
        let plugin = makeIntegration(for: analytics)
        let sourceConfig = makeSourceConfig(consentEntries: [gatedEntry()])
        // The event gate reads its destination config from source-config state at setup, so it is
        // ready for the first event — no wait needed before asserting a drop.
