...
struct EventFilteringPluginTests {
    ...
    // MARK: - Source Config Listener Tests

    /*
     Unlike the tests above, these drive the plugin through `sourceConfigState` rather than the
     `updateConfiguration` hook, so they cover the observation path itself. They guard the ordering
     bug where an integration added after the source config had already been dispatched left its
     filtering list empty and silently forwarded denied events.
     */

    /// "Bing Ads" in mock_source_config.json uses eventFilteringOption = "whitelistedEvents".
    private var sourceConfigDestinationKey: String { "Bing Ads" }
    private var whitelistedEvent: String { "form_submit" }
    private var nonWhitelistedEvent: String { "Track Event 3" }
    ...
    @Test("Given a source config dispatched before setup, When intercepting track events, Then filtering should be applied")
    func testLateSubscriberAppliesFiltering() async {
        ...
        let plugin = EventFilteringPlugin(key: sourceConfigDestinationKey)
        plugin.setup(analytics: analytics)

        await runAfter(0.5) {
            #expect(plugin.intercept(event: TrackEvent(event: whitelistedEvent)) != nil)
            #expect(plugin.intercept(event: TrackEvent(event: nonWhitelistedEvent)) == nil)
        }
    }

    @Test("Given a source config dispatched before setup and an identical refresh, When intercepting a track event, Then filtering should still be applied")
    ...
    @Test("Given a source config dispatched after setup, When intercepting track events, Then filtering should be applied")
